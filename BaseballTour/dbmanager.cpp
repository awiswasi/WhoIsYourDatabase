...
QVector<StadiumInfo> dbManager:: getStadiumInfo()
{
    ...
    if(query.exec())
    {
        while(query.next()) //these seem to be coming out in alphabetical order by default
        {
            ...
            myStructInfo.roofType = query.value(2).toString();
            myStructInfo.capacity = query.value(3).toInt();
            myStructInfo.distanceToCenter = query.value(4).toInt();
...

{
    ...
    query.prepare("Insert into mlb(Stadium, Team, roofType, Capacity, [distance to center], Surface, [Date Opened], [League Type], Typology) values ('"

                   +myStadiumInfo.stadiumName+ "', '" +myStadiumInfo.teamName+ "', '" +myStadiumInfo.roofType+ "', '" +QString::number(myStadiumInfo.capacity)+ "', '"

                   +myStadiumInfo.distanceToCenter+ "', '" +myStadiumInfo.surface+ "', '" +myStadiumInfo.dateOpen.toString(format)+ "', '" +myStadiumInfo.leagueType+ "', '"
...
//            qDebug() << teamName << " " << myString;
    ...
    {
        query.prepare("UPDATE MLB set roofType = ('"
